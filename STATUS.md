...
- Members now request linking with name and phone; phone input opens as a mask-shaped `___-____-____` field, accepts digits, and formats hyphens while typing; pending/approved requests hide duplicate request entry, and rejected requests can be retried.
...
- Admin `주간` and member `예약` detail screens use role-specific monthly calendars: admin shows operation handling priority, member shows own reservations and available days.
...
- 2026-06-17: Role-specific schedule calendar contract verified RED with `npm run check:layout`; GREEN verified with `npm run check:layout`, `npm run build`, `git diff --check`, and Playwright fixture screenshot at 320px.
